...
`machine_detection` | Configure the behavior when Nexmo detects that a destination is an answerphone. Set to `continue` to continue with call, or `hangup` to end call on machine detection. | ❎
